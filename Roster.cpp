...
std::string Roster::toString() const {
	...
	for(int i = 0; i<ROSTER_MAX;i++){
		if(this->students[i]){
			result+= "[";
			result+= std::to_string(this->students[i]->getPerm());
			result+= ",";
			result+= this->students[i]->getLastName();
			result+= ",";
			result+= this->students[i]->getFirstAndMiddleNames();
			result+= "]";
			if(this->students[i+1]){
				result+=",";
			}
			result+= "\n";
		}
...
int Roster::indexOfMaxPermAmongFirstKStudents(int k) const {
	int largestIndex = 0;
	for(int i = 1; i < k; i++){
		if(this->students[largestIndex]->getPerm() < this->students[i]->getPerm()){
			largestIndex = i;
		}
	}
	return largestIndex;
...
void Roster::sortByPermHelper(int k) {
	// swaps ma:q
	// perm from [0..k-1] with elem [k-1]
	Student* temp;
	int im = this->indexOfMaxPermAmongFirstKStudents(k);
	temp = this->students[k-1];
